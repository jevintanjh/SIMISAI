...
MODEL_PATH = os.getenv("MODEL_PATH", "models/yolov8n.pt")  # default to a small model
...
def get_model():
    global _model
    if _model is None:
        # Import here to keep container memory low until first request
...
def detect(req: DetectRequest) -> Dict[str, Any]:
    ...
    # CPU inference
    results = get_model().predict(source=np.array(img), conf=0.5, verbose=False, device="cpu")
