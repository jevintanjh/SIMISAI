# Quick System Architect Review for Windows
# Simple PowerShell review script

Write-Host "🔍 System Architect Quick Review" -ForegroundColor Blue
...
if (Test-Path "aws-deployment") {
    $files = Get-ChildItem -Path "aws-deployment" -Recurse -File -Include "*.js", "*.py", "*.ts"
    foreach ($file in $files) {
        $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
        if ($content -match "password.*=.*['\"].*['\"]|secret.*=.*['\"].*['\"]|key.*=.*['\"].*['\"]") {
            $secretsFound = $true
            break
        }
    }
